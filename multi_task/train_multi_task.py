    ...
    for epoch in tqdm(range(NUM_EPOCHS)):
        ...
        for batch in train_loader:
            ...
            if 'mgda' in params['algorithm']:
                ...
                #MGDA-UB可以使我们无需从头到尾的backporpogation，只需算到共享模型的最后一层。这样可以加快速度，又能的得到一个比骄傲好的解

                if approximate_norm_solution:
                    optimizer.zero_grad()
                    # First compute representations (z)
                    images_volatile = Variable(images.data, volatile=True) #volatile 相当于requiers—_grad=Flase
                ...
                # Frank-Wolfe iteration to compute scales. FrankWolfe用来解决约束优化问题，在论文的算法2中给出了Frank-Wolfe求解其的所有更新方程式

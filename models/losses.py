...
class IdentityLoss(torch.nn.Module):
    ...
    def forward(self, generated_image_features,reference_features):
        #generated_image_features and reference_features : dim [1, 4096]

        generated_image_features = generated_image_features.view(generated_image_features.shape[0], -1)  # 将特征转换为N*(C*W*H)，即两维
